...
export default function TransactionsPage() {
  ...
  const tabs: { id: Tab; label: string; icon: React.ElementType; count: number; color: string }[] = [
    { id: 'all', label: 'All Transactions', icon: FiFileText, count: apiTransactions.length, color: '#8A7060' },
    { id: 'sales', label: 'Sales', icon: FiShoppingCart, count: apiTransactions.filter((t) => t.type === 'selling').length, color: 'text-[#D4623A]' },
    { id: 'purchase', label: 'Purchases', icon: FiPackage, count: apiTransactions.filter((t) => t.type === 'purchase').length, color: 'text-[#D4623A]' },
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {/* Header */}
        <div className="relative mb-8 rounded-2xl bg-white dark:bg-[#1A1A1A] border #E5D8CC dark:border-[#222222] shadow-sm">
          {/* Background Pattern */}
          ...
          <div className="relative px-6 py-8 sm:px-8 sm:py-10">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight #3D2B1A dark:text-[#E0E0E0] flex items-center gap-3">
                    Transactions
                    {isLoading ? (
                  ...
                  <p className="#8A7060 dark:text-[#555555] text-sm mt-1 max-w-md font-medium">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                ...
                <button
                  ...
                  className="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium text-gray-700 dark:text-[#CCCCCC] #FAF7F3 dark:bg-[#222222]/50 hover:#EDE5DA dark:hover:bg-gray-700 border #E5D8CC dark:border-[#333333] transition-all shadow-xs"
                >
                ...
                <div className="relative">
                  ...
                  {showExportMenu && (
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-[#1A1A1A] rounded-xl shadow-2xl border #E5D8CC dark:border-[#222222] py-2 z-50">
                        {['pdf', 'excel', 'csv'].map((format) => (
                          <button
                            key={format}
                            onClick={() => handleExport(format as any)}
                            className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-[#CCCCCC] hover:#EDE5DA dark:hover:bg-gray-700 transition-colors"
                          >
                            <FiFileText className={`w-4 h-4 ${format === 'pdf' ? 'text-red-500' : format === 'excel' ? 'text-green-500' : 'text-[#D4623A]'}`} />
                ...
                <div className="relative">
                  ...
                  {showNewTransactionMenu && (
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-[#1A1A1A] rounded-xl shadow-2xl border #E5D8CC dark:border-[#222222] py-2 z-50 max-h-[70vh] overflow-y-auto">
                        <div className="px-4 py-2 border-b #E5D8CC dark:border-[#222222] mb-1">
                          <p className="text-xs font-semibold #8A7060 dark:text-[#555555] uppercase tracking-wide">Create Transaction</p>
                        </div>
                        {transactionMenuItems.map((item) => (
                          <button
                            ...
                            className="w-full flex items-center gap-3 px-4 py-2.5 hover:#FAF7F3 dark:hover:bg-gray-700 transition-colors"
                          >
                            <div className={`w-9 h-9 rounded-lg ${item.color} flex items-center justify-center shrink-0 shadow-sm`}>
                              <item.icon className="w-4 h-4 text-white" />
                            </div>
                            <div className="text-left">
                              <p className="font-medium text-sm #3D2B1A dark:text-[#E0E0E0]">{item.label}</p>
                              <p className="text-xs #8A7060 dark:text-[#555555]">{item.desc}</p>
                            </div>
                          </button>
                        ))}
                      </div>
        ...
        <Card className="p-4 sm:p-6 mb-6">
          {/* Quick Date Filters */}
          <div className="flex flex-wrap items-center gap-2 mb-4 pb-4 border-b #E5D8CC dark:border-[#222222]">
            <div className="flex items-center gap-2 mr-2">
              <FiCalendar className="w-4 h-4 text-gray-400" />
              <span className="text-sm font-medium #8A7060 dark:text-[#555555]">Period:</span>
            </div>
          ...
          {/* Advanced Filters */}
          {showFilters && (
            <div className="#FAF7F3 dark:bg-[#1A1A1A]/50 rounded-xl p-4 animate-in fade-in slide-in-from-top-2">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold #3D2B1A dark:text-[#E0E0E0]">Custom Date Range</h3>
                <button
                  onClick={() => setShowFilters(false)}
                  className="p-1.5 rounded-lg text-gray-400 hover:#8A7060 hover:#EDE5DA dark:hover:bg-gray-700"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-[#CCCCCC]">From</label>
                  <input
                    type="date"
                    value={startDate}
                    onChange={(e) => {
                    ...
                    className="w-full px-4 py-3 rounded-xl border-2 #E5D8CC dark:border-[#222222] bg-white dark:bg-[#1A1A1A] #3D2B1A dark:text-[#E0E0E0] focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-[#CCCCCC]">To</label>
                  <input
                    type="date"
                    value={endDate}
                    onChange={(e) => {
        ...
        <div className="relative">
          {isLoading && apiTransactions.length === 0 ? (
            <Card className="p-20 flex flex-col items-center justify-center">
              ...
              <p className="#8A7060 font-medium">Loading transactions...</p>
            </Card>
          ) : (
            <TransactionTable
