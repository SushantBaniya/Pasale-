...
function Modal({ open, onClose, title, children, wide = false }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div
        className={`bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full mx-4 ${wide ? 'max-w-2xl' : 'max-w-md'}`}
    ...
    <Modal open={open} onClose={onClose} title="Apply Discount">
      <div className="space-y-4">
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3 flex justify-between items-center">
          ...
          <span className="text-sm font-bold text-gray-900 dark:text-white">{fmt(subtotal)}</span>
        </div>
        <div>
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              <input
                ...
                className="w-full px-3 py-2.5 pr-8 border-2 border-blue-500 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none"
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-gray-400">%</span>
            </div>
            <button
              onClick={() => setLinked(!linked)}
              className={`w-9 h-9 flex items-center justify-center rounded-lg border-2 transition-colors ${linked ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'border-gray-200 dark:border-gray-700 text-gray-400'}`}
            >
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 pl-10 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-400"
              />
            </div>
          </div>
        </div>
...
function VATModal({ open, onClose, taxableAmount, onApply }: {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Apply VAT / Tax">
      <div className="space-y-4">
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3 flex justify-between items-center">
          <span className="text-sm font-medium text-gray-500">Taxable Amount</span>
          <span className="text-sm font-bold text-gray-900 dark:text-white">{fmt(taxableAmount)}</span>
        </div>
        <div>
          ...
          <div className="flex gap-2 mt-2">
            {['0', '5', '13', '15'].map(v => (
              <button
                key={v}
                onClick={() => setPct(v)}
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${pct === v ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'}`}
              >
                {v}%
              </button>
            ))}
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-600 rounded-xl hover:bg-gray-50 transition-colors">
            Cancel
          </button>
          <button
    ...
    <Modal open={open} onClose={onClose} title="Add New Item" wide>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Item Category</label>
            <select
              value={form.category}
              onChange={e => set('category', e.target.value)}
              className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
            >
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Item Type</label>
            <div className="flex gap-2">
              {['Product', 'Service'].map(t => (
                <button
                  key={t}
                  onClick={() => set('item_type', t)}
                  className={`flex-1 py-2.5 text-sm font-semibold rounded-xl border-2 transition-colors ${form.item_type === t ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'border-gray-200 dark:border-gray-700 text-gray-500'}`}
                >
                  {t}
                </button>
              ))}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Sales Price</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="0"
                className="w-full pl-9 pr-12 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-400">/PCS</span>
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Purchase Price</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-400">Rs.</span>
              <input
                type="number"
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="0"
                className="w-full px-3 pr-12 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-400">PCS</span>
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Primary Unit</label>
            <select
              value={form.unit}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Item Code</label>
            <div className="flex gap-2">
              <input
                type="text"
                ...
                className="flex-1 px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
              />
              <button
                onClick={generateCode}
                className="px-3 py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors whitespace-nowrap"
              >
                Generate
              </button>
            </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">HS Code</label>
            <input
              type="text"
              value={form.hs_code}
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-600 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            Cancel
          </button>
          <button
            onClick={handleSave}
...
function EditItemModal({ open, onClose, item, onSave }: {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Edit — ${item.name}`}>
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Rate / Price</label>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-gray-400">Rs.</span>
            <input
              type="number"
              ...
              className="w-full pl-10 pr-3 py-2.5 border-2 border-blue-500 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none"
            />
          </div>
        </div>
        <div>
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-600 rounded-xl hover:bg-gray-50 transition-colors">Cancel</button>
          <button
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">
              Invoice No
              <button className="ml-2 text-blue-500 font-bold normal-case">Manual</button>
            </label>
            <input
              type="text"
              value={invoiceNo}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2.5 pr-10 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
              />
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Bill To</label>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
              />
            </div>
          </div>
          <div>
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Notes or Remarks</label>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            rows={3}
            className="w-full px-3 py-2.5 border-2 border-blue-400 rounded-xl text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none resize-none"
          />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Attach Images</label>
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className="w-16 h-16 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl flex flex-col items-center justify-center text-gray-400 hover:border-blue-400 hover:text-blue-500 transition-colors"
            >
              <FiCamera className="w-5 h-5" />
            </button>
            {images.map((f, i) => (
              <div key={i} className="w-16 h-16 rounded-xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-xs text-blue-600 font-medium truncate px-1 text-center">
                {f.name.slice(0, 8)}
              </div>
            ))}
          </div>
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={onClose} className="px-5 py-2.5 border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-600 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            Cancel
          </button>
          <button
            onClick={() => handleConfirm(false)}
            disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-gray-300 dark:border-gray-700 text-sm font-bold text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 transition-colors"
          >
          ...
          <button
            ...
            disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 disabled:opacity-50 transition-colors shadow-lg shadow-blue-500/20"
          >
...
export default function QuickPOSPage() {
  ...
  return (
    <div className="flex gap-0 h-[calc(100vh-64px)] bg-gray-50 dark:bg-gray-950 overflow-hidden">

      {/* ── LEFT: Product Catalog ── */}
      <div className="flex-1 flex flex-col min-w-0 px-6 py-5 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between mb-5">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Quick POS</h1>
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 w-56 transition-all"
              />
            </div>
            <button
              onClick={() => setShowAddItem(true)}
              className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-sm font-semibold rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors shadow-sm"
            >
              ...
              {filtered.map(p => {
                ...
                return (
                  <div key={p.id} className="relative">
                    {/* Info button */}
                    <button className="absolute top-2 right-2 z-10 w-6 h-6 flex items-center justify-center text-gray-300 hover:text-gray-500 transition-colors">
                      <FiInfo className="w-3.5 h-3.5" />
                    </button>

                    {inCart ? (
                      /* Selected state with qty controls */
                      <div className="bg-white dark:bg-gray-900 border-2 border-blue-500 rounded-2xl p-4 flex flex-col items-center shadow-sm">
                        <div
                        ...
                        <p className="text-xs font-bold text-gray-900 dark:text-white text-center mb-0.5 truncate w-full">{p.product_name}</p>
                        <p className="text-xs text-gray-400 mb-2">Qty: {p.quantity} PCS</p>
                        <p className="text-xs font-bold text-gray-700 dark:text-gray-300 mb-3">{fmt(p.unit_price)}/PCS</p>
                        <div className="flex items-center gap-3 w-full justify-center">
                          <button
                            onClick={() => { if (inCart.quantity <= 1) removeItem(p.id); else updateQty(p.id, -1); }}
                            className="w-7 h-7 flex items-center justify-center border border-gray-200 dark:border-gray-700 rounded-lg text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                          >
                            <FiMinus className="w-3 h-3" />
                          </button>
                          <span className="text-sm font-bold text-gray-900 dark:text-white w-4 text-center">{inCart.quantity}</span>
                          <button
                          ...
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center border border-gray-200 dark:border-gray-700 rounded-lg text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
                          >
                      ...
                      <button
                        ...
                        disabled={outOfStock}
                        className={`w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 flex flex-col items-center hover:border-blue-300 dark:hover:border-blue-700 hover:shadow-md transition-all duration-150 ${outOfStock ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}
                      >
                        <div
                        ...
                        <p className="text-xs text-gray-400 mb-2">Qty: {p.quantity} PCS</p>
                        <p className="text-xs font-bold text-gray-700 dark:text-gray-300 mb-3">{fmt(p.unit_price)}/PCS</p>
                        <span className="w-full py-1.5 bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-xs font-semibold rounded-lg text-center">
      ...
      {/* ── RIGHT: Billing Panel ── */}
      <div className="w-[320px] shrink-0 bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 flex flex-col">
        {/* Billing Header */}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-10">
              ...
              <p className="text-sm font-bold text-gray-400 dark:text-gray-600">No Billing Items</p>
              <p className="text-xs text-gray-300 dark:text-gray-700 mt-0.5">Select items to record a sale</p>
            </div>
          ) : (
            items.map(item => (
              <div key={item.id} className="border border-gray-100 dark:border-gray-800 rounded-xl p-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-6 h-6 flex items-center justify-center border border-gray-200 dark:border-gray-700 rounded-md text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                    >
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 px-5 py-4 space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Sub Total</span>
            <span className="text-sm font-bold text-gray-900 dark:text-white">{fmt(subtotal)}</span>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowDiscount(true)} className="text-gray-400 hover:text-blue-500 transition-colors">
                <FiEdit2 className="w-3 h-3" />
              </button>
              {finalDiscount > 0 && (
            ...
            {finalDiscount > 0 ? (
              ...
              <button onClick={() => setShowDiscount(true)} className="text-xs font-semibold text-blue-500 hover:text-blue-600">
                + Discount
              </button>
            )}
          </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-500">VAT {taxPct > 0 ? `${taxPct}%` : ''}</span>
              <button onClick={() => setShowVAT(true)} className="text-gray-400 hover:text-blue-500 transition-colors">
                <FiEdit2 className="w-3 h-3" />
              </button>
              {taxPct > 0 && (
            ...
            {taxAmount > 0 ? (
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{fmt(taxAmount)}</span>
            ) : (
              <button onClick={() => setShowVAT(true)} className="text-xs font-semibold text-blue-500 hover:text-blue-600">
                + Tax
              </button>
            )}
          </div>
          ...
          <div className="flex justify-between items-center">
            <span className="text-xs font-semibold text-blue-500 cursor-pointer hover:text-blue-600">+ Additional Charges</span>
          </div>
