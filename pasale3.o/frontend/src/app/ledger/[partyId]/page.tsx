...
export default function LedgerPage() {
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="p-12 text-center">
          <p className="text-[#475569] dark:text-[#44454F]">{t('parties.noParties')}</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F7FAFC] dark:bg-[#0D0E12] pt-4 pb-6 sm:pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 text-xs rounded-lg border border-gray-300 dark:border-[#2A2B36] bg-white dark:bg-[#15161C] text-[#1E293B] dark:text-[#EAE5DF]"
            >
          ...
          <div>
            ...
            <div className="relative">
              <FiSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-[#475569] w-4 h-4" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={t('ledger.searchDescription')}
                className="w-full pl-9 pr-4 py-2 text-xs border rounded-lg bg-white dark:bg-[#15161C] text-[#1E293B] dark:text-[#EAE5DF] border-gray-300 dark:border-[#2A2B36] focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
        </div>
        ...
        <Card className="p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={chartData}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="balance" stroke="#F2DD50" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {ledgerEntries.map((entry) => (
                <tr
                  key={entry.id}
                  className="border-b border-[#E2E8F0] dark:border-[#1C1D24] hover:bg-[#F7FAFC] dark:hover:bg-gray-800/50"
                >
                  <td className="py-3 px-4 text-[#1E293B] dark:text-[#EAE5DF]">
                    {d(entry.date)}
                  </td>
                  <td className="py-3 px-4 text-[#1E293B] dark:text-[#EAE5DF] capitalize">
